...
- ブロック対象として選択したアプリ／カテゴリの識別子（FamilyControls API による不透明なトークン。アプリ名や使用履歴は含まれません）
...
| 機能 | 送信される情報 | 送信先 |
|---|---|---|
| AI問題生成 | ユーザーが入力したテーマ文字列 | 開発者運用の Cloudflare Workers サーバー → Google Gemini API |
| 写真からの取込（AI後処理） | OCR で認識されたテキスト | 同上 |
| プレミアム購入 | App Store 決済レシート | Apple, RevenueCat（本番リリース時） |

これらの通信内容には、ユーザー個人を特定する情報は含まれません。
